...
## ADR #70 — Plane V: a resumable, obligation-driven case investigator

**Date:** 2026-08-07 · **Branch:** v2-persons · **Status:** Accepted

**Complements:** ADR #43 (compliance gap analysis). Does not supersede it.

### Context

Every plane shipped so far is reactive. Plane II answers a query; Plane III
scores those answers; Plane IV displays them. Nothing in the system asks a
question nobody typed.

That is the actual gap, and it is not a retrieval gap. Three findings the
operator derived by hand over ~100 hours — a power of attorney exhausted by
its own execution, a defence that collapses under the symmetry of the doctrine
it invokes, a ledger never produced — are all *in* the corpus. The pipeline
never surfaced them because a RAG flow waits to be asked, and the questions
that matter are generated by comparing **what should be in the documents**
against **what is**.

That comparison needs three things a RAG flow structurally lacks: a fixed
denominator of expected behaviours, persistent state across many small
evaluations, and an adversarial pass that attacks its own conclusions.

**Why this is not `rag/compliance.py` renamed.** The two are adjacent and the
...
| Adversarial | none | `confounders`, catalog-seeded |
...
satisfied" from "never evaluated", because an obligation whose article failed to
retrieve produces no output at all. A catalog is a fixed denominator, and that
is the entire epistemic basis for treating **absence** as a signal. Compliance
remains the Opus-class one-shot deliverable; this is a different instrument.

**Absence-of-evidence epistemics.** An absence-driven system's characteristic
failure is claiming something did not happen when the record was never examined.
So one predicate yields two outcomes that are never conflated: `gap` (trigger
fired, scope covered, zero matches — "not performed") and `unverifiable`
(trigger fired, scope **not** covered — "the record needed to test this is not
in the dossier"). The second is a deliverable in its own right, a
document-request list, and it is what makes it structurally impossible to
upgrade "we didn't look" into "it didn't happen".

Measured, not assumed: `vitrine` has **no `coverage.jsonl`** and **no
`extracted/*.json` sidecars** — only `private` does (93 coverage rows, 55
sidecars). So on the public case `coverage_known=False` and every gap caps at
T5. That is correct and demonstrable, not a defect to paper over.

**The citation correction.** An early draft of the catalog anchored a
unanimity obligation to "Code civil, art. 730-4 al. 2". Read out of
`data/chunks.csv`: `cc-730-4` (`LEGIARTI000006430899`, `etat=VIGUEUR`, 281
chars) is a **single alinéa**, and it *enables* release — heirs named in the
acte de notoriété are deemed, as against third-party holders, to have free
disposal of estate funds "dans la proportion indiquée à l'acte". There is no
alinéa 2 and no unanimity requirement. The unanimity limb is `cc-815-3` in
fine, which is in the corpus: "le consentement de tous les indivisaires est
requis pour effectuer tout acte qui ne ressortit pas à l'exploitation normale
des biens indivis". The `search` pass catches exactly this class of error with
zero network calls, and it caught this one before any code existed.

Related: `cc-1204` (porte-fort), `cc-1344` (mise en demeure), `cc-494-12`
(habilitation familiale) and `cc-1231-1` are **absent** from the 792-chunk,
9-source corpus. Obligations anchored to them ship with `chunk_id: null` plus a
`legiarti_id`, and `search` emits `citation_unverifiable` rather than the
catalog test failing — the test keeps its teeth without blocking authoring.

**The fork decision.** `scripts/local_audit/` already proves this harness shape:
passes as plain functions, id-keyed findings with `first_seen`/`last_seen`,
resolve-on-fix via `reconcile_pass`, and a content-hash cache invalidated by
`prompt_version`. Plane V forks four of its modules rather than importing them,
for two reasons that are structural rather than stylistic:

1. **Packaging makes importing impossible.** `pyproject.toml` ships
   `packages = ["ingestion", "rag", "eval", "monitoring"]`; `scripts/` is not in
   the wheel. A shipped `investigator` importing `scripts.local_audit` would
   `ImportError` on install. The alternatives — shipping `scripts/` with its
   ruff/radon/vulture subprocess assumptions, or hoisting `local_audit` into a
   shipped package — are a rename across 12 modules for no user-visible benefit.
2. **The path coupling is structural.** `findings.py` and `cache.py` read
   `config.FINDINGS_JSONL` / `config.CACHE_DB` as **module globals at call
   time**. Plane V needs per-case paths, so generalising means threading a store
   handle through ~20 call sites in a working system. That is a
   behaviour-touching refactor, deliberately out of scope.

`scripts/local_audit/` is therefore **not** generalised. The anti-drift
mitigation is that `make_id` and `make_key` stay formula-identical and a test
imports both modules and asserts they agree on a fixed corpus, so a divergence
becomes a decision rather than a discovery.

One deliberate deviation from the fork source: `store.save_all` writes tmp +
`os.replace`. The original's `write_text` truncates before writing.
`local_audit` survives that because nobody kills it; this harness is *designed*
to be killed, and without atomicity its idempotence claim is false.

**Privacy.** Committed: the engine, a generic person-free catalog, and the
`vitrine` findings and digest. Not committed: the `private` catalog and every
`private` finding, covered automatically by ADR #66's ignore-by-default rule —
no new privacy surface, because state lives under
`data/dossier/{case_id}/investigation/`. `ALLOW_OUTBOUND_CASES` excludes
`private` outright. A contract-clause obligation cannot be citation-verified
without a case, so contract clauses live only in per-case overlays and the
committed catalog is statute and deontology only.

**Files outside `investigator/` that must change**, and why each is
unavoidable: `pyproject.toml` (a new top-level package absent from the wheel
`packages` list is not installed); `scripts/local_audit/config.py` —
`SCAN_DIRS`, `PLANE_DIRS`, `CROSS_PLANE_ALLOWED_IMPORTERS` (otherwise the new
plane escapes the convention checks entirely); `.gitignore` (`vitrine` is
un-ignored, so `investigation/cache.sqlite3` would become a committed binary
churning every run). `CROSS_PLANE_ALLOWED_SYMBOLS` is deliberately **not**
widened — that would weaken the check for every plane to legitimise one. The
2-3 new low-confidence convention findings for Plane V's `ingestion.clients`
and `ingestion.dossier.facts` imports are expected, are the check working as
its docstring describes, and are recorded here as reviewed.

### Decision

Add `investigator/` as Plane V, under eight commitments.

1. **Passes are plain module-level functions** returning
   `PassResult(findings, complete)`; the orchestrator hard-wires them by name;
   `_record` is the only framework seam. `PassResult` replaces
   `slimming.LAST_DIVERGENCE_COMPLETE` — a module global mutated by one function
   and read by another is not reentrant, and per-case runs may be concurrent.
2. **`claim` text is identity.** Volatile data goes in `evidence`. `confounders`,
   `tier_history` and `calibration` sit **outside** `upsert_many`'s update set,
   so a Check re-run can never erase adversarial work — the same mechanism that
   already preserves `first_seen`.
3. **Deterministic first.** No LLM call may answer a question a predicate
   answers. In Phase 2 the model gets exactly three jobs: one quote against one
   clause; one pair of quotes; confounder prose. It never sees the obligation
   set, never sees the corpus, and never decides membership.
4. **A partial cycle never reconciles.** Budget cutoff and transport failure must
   stay distinguishable from "fixed".
5. **A `None` verdict is never cached.** "No verdict" is not "no".
6. **`store.save_all` is atomic** via `os.replace`.
7. **One externalisation gate function, one caller.** `private` is not
   outbound-eligible; `person_id`s never survive onto an outbound path.
8. **Budget**: per-cycle local-call cap, per-cycle and per-session USD ceilings
   computed by `ingestion.clients.estimate_cost_usd` (ADR #68 — rates are not
   redefined outside that module). Cloud is reachable only from `attack` and
   only via `may_escalate`, default-off.

Delivery is staged. **Phase 1 (to 11 Aug): all five passes ship and none calls
an LLM.** Every model tier is an additive adjudication layer over a pass that
already works deterministically, so a full cycle costs $0.00 and cannot fail
because Ollama is down. **Phase 2 (12-14 Aug):** the local qwen3 layers.

Contracts live in `docs/investigator-spec.md`, per ADR #69.

### Consequences

- A full deterministic cycle over `vitrine` is seconds and $0.00. Compare
  `generate_compliance_matrix` at ~$22 for one run of the same case.
- `data/dossier/vitrine/investigation/` becomes a committed, reviewable
  artifact — findings and digest, produced by a pipeline a reviewer can rerun.
- `investigator/` comes under the section-divider and public-docstring
  conventions, and its `chunks.csv` reads under the `keep_default_na=False`
  check.
- One new test file. The existing 251 stay green; no existing module's behaviour
  changes.
- With `adjudicate: none` everywhere in Phase 1, `check` will produce some
  false-positive gaps where a satisfying fact is phrased outside a term list.
  That is why every entry ships `confounders_seed` and why gaps cap at T3 — T5
  on `vitrine`. `Evaluation.candidate_fact_ids` already exists so the Phase 2
  rescue tier plugs in without touching another module.
- `Fact.mentioned_person_ids` stays absent. Plane V gets the fact→person edge by
  inverting `persons.jsonl`'s `role_assignments[].evidence_fact_ids` — measured
...
- (a) `check` LLM rescue tier — qwen3:14b, self-consistency, can only flip
  `gap → satisfied`, never create a gap.
- (b) `contradict` adjudication — qwen3:30b over the already-stable pair list.
  `duplication.py` documents that model failing a comparable pairwise task 3/3
  at both sizes; the pass keeps emitting Layer-0 collisions regardless, so the
  layer is revertible with nothing else lost.
- (c) `attack` generative confounders, and cloud escalation on by default.
- (d) Live PISTE citation validation (Layer 0 ships; the network layer is
  default-off).
- (e) `rag.retrieve` for *unanchored* obligation discovery. Deliberately not
  imported now: the catalog carries explicit `chunk_id`s, and a long-running
  loop holding BGE-M3 alongside a local model on a 12 GB card is the contention
  `scripts/local_audit/config.py` already documents.
- (f) Multi-case watch — one process per case for now.
- (g) Any Streamlit surface. Plane IV is untouched.
- (h) Calibration-log UI.

### Rejected

- **Generalising `scripts/local_audit/` for both callers.** Blocked by the wheel
  `packages` list, and it would be a behaviour-touching refactor of working dev
  tooling. Two small honest modules beat one with a union schema.
- **A graph database.** Plane Ib already emits entities, events and evidence as
  JSONL. What was missing is obligations and findings, which are two more files,
  not a new storage engine.
- **Free-form LLM investigation reports.** They encode model bias in longer
